...
>基本包装类型Boolean、Number 和 String在读取基本类型值后，后台会创建一个对应的对象，从而能够调用一些内置方法如substring等。
...
Query是一个js库，主要提供的功能是选择器，属性修改和事件绑定等等。  
